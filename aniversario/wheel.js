...
(function (w) {
  ...
  const COLORS = {
    L1: ["#FF6B03", "#C84E00"],
    L2: ["#1A1A1A", "#2C4A6E", "#1A1A1A", "#3A5F7D", "#1A1A1A", "#243F5E"],
    L3: ["#C9A000", "#7A5C00", "#E8C200", "#9A7200", "#F0CC00"],
  };

  const RING = {
    outer: { r1: 226, r2: 170 },
    middle: { r1: 166, r2: 110 },
    inner: { r1: 106, r2: 54 },
  };
  ...
  function getVisualSegs(level) {
    ...
    for (let i = 0; i < n; i++) {
      base.forEach(function (s) {
        result.push({ id: s.id, name: s.name, weight: s.weight });
      });
    }
    return result;
  }
  ...
  function drawHighlights(angles, resultIds, alpha) {
    ...
    pairs.forEach(function (p) {
      ...
      const total = segs.reduce(function (s, x) {
        return s + x.weight;
      }, 0);
  ...
  function spin(resultIds, onDone) {
    ...
    startAngle = {
      outer: currentAngle.outer,
      middle: currentAngle.middle,
      inner: currentAngle.inner,
    };
    ...
    targetAngle = {
      ...
      middle:
        Math.PI * 2 * (7 + Math.floor(Math.random() * 4)) + offsets.middle,
    ...
    function animate(ts) {
      ...
      currentAngle = {
        ...
        middle:
          startAngle.middle + (targetAngle.middle - startAngle.middle) * e,
      ...
      if (window._bgorSound) {
        ...
        if (newCount > lastTickCount) {
          ...
          const burst = Math.min(newCount - lastTickCount, 2);
