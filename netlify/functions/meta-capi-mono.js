...
const PIXEL_ID = '968707005757335';
...
exports.handler = async (event) => {
    ...
    const accessToken = process.env.META_ACCESS_TOKEN_MONO || process.env.META_ACCESS_TOKEN;
    if (!accessToken) {
        console.warn('[meta-capi-mono] META_ACCESS_TOKEN_MONO no configurado; omitiendo evento');
        return {
            statusCode: 200,
        ...
        data: [
            {
                ...
                event_source_url: data.sourceUrl || 'https://bgor.com.co/monogastricos',
                user_data: userData,
                custom_data: {
    ...
    try {
        ...
        console.log('[meta-capi-mono] Status:', response.status, '| Body:', responseText);
        ...
        console.error('[meta-capi-mono] Error:', error.message);
        return {
            statusCode: 500,
            headers: { 'Access-Control-Allow-Origin': '*' },
